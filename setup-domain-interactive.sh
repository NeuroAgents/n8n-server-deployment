...
# Скрипт для настройки домена и SSL для n8n на сервере (интерактивная версия)
...
scp remote_setup.sh $SSH_USER@$SERVER_IP:/tmp/

# Выполняем скрипт на сервере
echo "Запускаю скрипт на сервере..."
ssh -t $SSH_USER@$SERVER_IP "bash /tmp/remote_setup.sh"

# Удаляем временный файл
rm remote_setup.sh

echo "Проверьте, что n8n теперь доступен по адресу: https://$DOMAIN" 